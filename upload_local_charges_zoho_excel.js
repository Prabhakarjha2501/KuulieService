...
const getAndProcessLocalChargesForAll = async () => {
    ...
    console.log("allClientLocalChargesFolderId======>>>>", allClientLocalChargesFolderId)
    const clientFoldersResponse = await getFolders(zohoAccessToken, allClientLocalChargesFolderId);
    const clientFolders = clientFoldersResponse?.data?.FOLDER;
    console.log('clientFolders.length', clientFolders.length)
            ...
            for (let carrierInnerIndex = 0; carrierInnerIndex < carrierInnerFolders.length; carrierInnerIndex++) {
                ...
                for (let carrierFileIndex = 0; carrierFileIndex < carrierFiles.length; carrierFileIndex++) {
                    ...
                    if (carrierIndex === 0 && carrierInnerIndex === 0) {
                        ...
                        const detail = await processExcelDocument(filePath, clientData.dbConnectionString, "Local Charges", 'ZOHO_SYNC_LOG_' + rateFile.DOCNAME + '.txt')
...
getAndProcessLocalChargesForAll().then(res => console.log(res)).catch(() => console.log('ERROR'));